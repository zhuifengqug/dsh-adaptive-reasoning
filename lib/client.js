...
`
    ...
    function apply(ctx) {
      ...
      const show = (sessionId, anchor) => {
        ...
        host.style.left = `${Math.max(11, Math.min(rect.right - width, window.innerWidth - width - 11))}px`
        host.style.top = `${rect.top > 160 ? Math.max(11, rect.top - 156) : Math.min(window.innerHeight - 156, rect.bottom + 7)}px`
