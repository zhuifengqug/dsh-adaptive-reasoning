  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector(`style[data-plugin-css="${STYLE_ID}"]`) === null) {
      ...
      style.textContent = `
        ...
        .dar-track { position: relative; display: grid; align-items: end; height: 32px; }
        .dar-track::before { position: absolute; right: 9px; bottom: 5px; left: 9px; height: 3px; border-radius: 999px; background: var(--dsw-alias-border-l2); content: ''; }
        .dar-fill { position: absolute; left: 9px; bottom: 5px; height: 3px; border-radius: 999px; background: var(--dsw-alias-state-info-primary); pointer-events: none; }
        .dar-flame-canvas { position: absolute; left: 9px; right: 9px; top: 0; bottom: 0; pointer-events: none; }
        .dar-range { position: relative; z-index: 1; width: 100%; height: 32px; margin: 0; cursor: pointer; accent-color: var(--dsw-alias-state-info-primary); }
        .dar-range--hidden { opacity: 0; }
        .dar-range:disabled { cursor: progress; }
        .dar-labels { display: flex; justify-content: space-between; margin: 0 9px 4px; color: var(--dsw-alias-label-caption); font-size: 11px; line-height: 16px; pointer-events: none; }
    ...
    class FlameRenderer {
      ...
      spawnParticle(originX, originY, intensity) {
        ...
        p.active = true
        p.x = originX + (Math.random() - 0.5) * (6 + intensity * 16)
        p.y = originY + (Math.random() - 0.5) * 6
        p.vx = (Math.random() - 0.5) * 0.5
        p.vy = -0.4 - Math.random() * (0.5 + intensity * 0.9)
        p.maxLife = 0.25 + Math.random() * (0.35 + intensity * 0.4)
        p.life = p.maxLife
        p.size = 0.7 + Math.random() * (1 + intensity * 1.4)
      }
      ...
      palette(intensity) {
        // Warm ember -> orange -> red/white-hot as intensity grows.
        const hue = 32 - intensity * 28
        const sat = 55 + intensity * 45
        const lit = 55 + intensity * 26
        return { hue, sat, lit }
      }
      ...
      draw(staticOnly) {
        ...
        if (intensity < 0.02) {
          // Extinguished ember at the far left.
          const x = 3 + intensity * (width - 6)
          ctx.beginPath()
          ctx.arc(x, midY, 2, 0, Math.PI * 2)
          ctx.fillStyle = `hsla(${pal.hue}, ${pal.sat}%, ${pal.lit}%, 0.5)`
          ctx.fill()
          return
        }

        // Horizontal flame band: the leading edge follows the slider value
        // while the band hugs the track (no tall vertical plume).
        ctx.globalCompositeOperation = 'lighter'
        const front = Math.max(4, intensity * width)
        const N = Math.max(20, Math.floor(front / 3))
        const halfBase = Math.max(3, height * 0.5 - 4)

        const edge = (t) => {
          const x = t * front
          const taper = Math.sin(Math.PI * Math.min(1, x / front))
          const jitter = staticOnly ? 0 : Math.sin(this.time * 0.16 + t * 15) * (1.2 + intensity * 2.2)
          const halfH = Math.max(1.5, halfBase * taper + jitter)
          return { x, halfH }
        }

        ctx.beginPath()
        for (let i = 0; i <= N; i++) {
          const { x, halfH } = edge(i / N)
          const y = midY - halfH
        ...
        // Horizontal temperature gradient: ember tail -> bright front.
        const grad = ctx.createLinearGradient(0, 0, front, 0)
        grad.addColorStop(0, `hsla(${pal.hue}, ${pal.sat}%, ${pal.lit - 12}%, 0.18)`)
        grad.addColorStop(0.55, `hsla(${pal.hue - 8}, ${pal.sat}%, ${pal.lit}%, 0.7)`)
        grad.addColorStop(1, `hsla(${pal.hue - 16}, ${pal.sat}%, ${Math.min(100, pal.lit + 16)}%, 0.95)`)
        ctx.fillStyle = grad
        ctx.fill()

        // Bright leading edge.
        const edgeR = Math.min(18, height * 0.7)
        const core = ctx.createRadialGradient(front, midY, 1, front, midY, edgeR)
        core.addColorStop(0, `hsla(${pal.hue - 18}, 100%, 86%, 0.9)`)
        core.addColorStop(1, `hsla(${pal.hue - 18}, 100%, 70%, 0)`)
        ctx.fillStyle = core
        ctx.beginPath()
        ctx.arc(front, midY, edgeR, 0, Math.PI * 2)
        ctx.fill()

        if (staticOnly) return

        // Spark particles rising off the band surface.
        if (intensity > 0.15 && Math.random() < 0.2 + intensity * 0.4) {
          this.spawnParticle(front - Math.random() * front * 0.5, midY, intensity)
        }
    ...
    function EffortPanel({ api, remote, sessionId, close }) {
      ...
      return React.createElement('div', { className: 'dar-panel', role: 'dialog', 'aria-label': copy.title },
        ...
        React.createElement('div', { className: 'dar-track' },
          React.createElement('div', { className: 'dar-fill', style: { width: `calc(${percent}% - ${(percent * 0.18).toFixed(2)}px)` } }),
