  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector(`style[data-plugin-css="${STYLE_ID}"]`) === null) {
      ...
      style.textContent = `
        ...
        .dar-panel { width: min(228px, calc(100vw - 18px)); pointer-events: auto; padding: 10px 10px 9px; border: 1px solid #6a2c1e; border-radius: 0; background: #150a08; box-shadow: inset 0 0 0 1px #150a08, inset 0 0 0 2px #6a2c1e, 4px 4px 0 0 rgba(0, 0, 0, 0.55); color: #f0d8c8; font-family: 'PressStart2P', 'Zpix', 'Courier New', monospace; image-rendering: pixelated; }
        .dar-heading { display: flex; align-items: baseline; justify-content: space-between; gap: 8px; margin: 0 0 7px; }
        .dar-title { color: #b98a76; font-size: 8px; line-height: 12px; }
        .dar-value { overflow: hidden; color: #f0d8c8; font-size: 9px; line-height: 13px; text-overflow: ellipsis; white-space: nowrap; text-shadow: 1px 1px 0 rgba(0, 0, 0, 0.6); }
        .dar-description { min-height: 12px; margin: -1px 0 7px; color: #b98a76; font-size: 7px; line-height: 12px; }
        .dar-track { position: relative; display: grid; align-items: end; height: 24px; border: 1px solid #2e2926; border-radius: 0; background: #070606; box-shadow: inset 0 0 0 1px #070606; image-rendering: pixelated; }
        .dar-cells { position: absolute; inset: 4px; display: flex; gap: 1px; pointer-events: none; }
        .dar-cell { flex: 1; min-width: 3px; border: 1px solid #292522; background: #11100f; }
        ...
        .dar-range { position: relative; z-index: 1; width: 100%; height: 24px; margin: 0; cursor: pointer; }
        .dar-range--hidden { opacity: 0; }
        .dar-range:disabled { cursor: progress; }
        .dar-labels { display: flex; justify-content: space-between; margin: 0 2px 4px; color: #b98a76; font-size: 7px; line-height: 10px; pointer-events: none; }
        .dar-tick { overflow: hidden; max-width: 40px; text-overflow: ellipsis; white-space: nowrap; }
        .dar-tick:first-child { text-align: left; }
        .dar-tick:last-child { text-align: right; }
        .dar-status { padding: 6px 0 0; color: #b98a76; font-size: 8px; line-height: 12px; }
        .dar-error { color: #ff6a4a; }
        .dar-declare { margin-top: 7px; padding: 6px; border: 1px solid #3a1c14; background: #1a0d0a; }
        .dar-declareHint { display: block; margin-bottom: 6px; color: #b98a76; font-size: 7px; line-height: 12px; }
        .dar-declareRow { display: flex; align-items: center; justify-content: space-between; gap: 6px; }
        .dar-declareButton { flex: 0 0 auto; padding: 4px 8px; border: 1px solid #6a2c1e; border-radius: 0; background: #2a1410; color: #f0d8c8; font: inherit; font-size: 8px; cursor: pointer; }
        .dar-declareButton:hover:not(:disabled) { background: #3a1c14; }
        .dar-declareButton:disabled { color: #6a4a3e; cursor: progress; }
        .dar-menu-entry { display: flex; align-items: center; gap: 6px; width: 100%; min-height: 26px; margin-top: 3px; padding: 5px 6px; border: none; border-radius: 0; background: #1a0d0a; color: #f0d8c8; font-family: 'PressStart2P', 'Zpix', 'Courier New', monospace; font-size: 8px; line-height: 12px; text-align: left; cursor: pointer; }
...
`
    ...
    function apply(ctx) {
      ...
      const show = (sessionId, anchor) => {
        // Capture the anchor rect BEFORE hiding the menu, otherwise the
        // menuitem collapses to all-zero coords and the panel jumps to 0,0.
        const rect = anchor.getBoundingClientRect()
        for (const menu of document.querySelectorAll('[role="menu"]')) {
          if (menu.offsetParent !== null) menu.style.display = 'none'
        }
        const width = Math.min(228, window.innerWidth - 18)
        host.style.left = `${Math.max(9, Math.min(rect.right - width, window.innerWidth - width - 9))}px`
        host.style.top = `${rect.top > 140 ? Math.max(9, rect.top - 132) : Math.min(window.innerHeight - 132, rect.bottom + 6)}px`
      ...
      const onClick = (event) => {
        ...
        // Guard against the synthesized trigger.click() below re-entering us.
        ...
        // The official menu opens with two rows: "模型" then "推理等级".
        // Match the second one precisely so the panel anchors to it.
        let row = null
        const menu = target.closest('[role="menu"]')
        if (menu !== null) {
          const labels = Array.from(menu.querySelectorAll('button[role="menuitem"]'))
          row = labels.find(item => {
            const text = (item.textContent || '').trim()
            return [...EFFORT_LABELS].some(label => text.startsWith(label))
          }) ?? null
        }
        if (row === null) { hide(); return }
        event.preventDefault(); event.stopPropagation()
        // Find the menu's owning trigger so we can "click" it to close the
        // official menu cleanly (otherwise it would sit behind the panel).
        const menuEl = row.closest('[role="menu"]')
        const trigger = menuEl?.parentElement?.querySelector(
          ':scope > button[aria-haspopup="menu"], :scope > [role="presentation"] > button[aria-haspopup="menu"]'
        )
        show(sessionId, row)
        if (trigger instanceof HTMLElement) {
          suppressClick = true
          trigger.click()
          // Release the guard after the click bubbles through the document.
