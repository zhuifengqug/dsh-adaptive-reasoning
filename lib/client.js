  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector(`style[data-plugin-css="${STYLE_ID}"]`) === null) {
      ...
      style.textContent = `
        ...
        .dar-panel { width: min(304px, calc(100vw - 24px)); pointer-events: auto; padding: 14px 14px 12px; border: 2px solid #6a2c1e; border-radius: 0; background: #150a08; box-shadow: inset 0 0 0 2px #150a08, inset 0 0 0 4px #6a2c1e, 6px 6px 0 0 rgba(0, 0, 0, 0.55); color: #f0d8c8; font-family: 'PressStart2P', 'Zpix', 'Courier New', monospace; image-rendering: pixelated; }
        .dar-heading { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; margin: 0 0 10px; }
        .dar-title { color: #b98a76; font-size: 11px; line-height: 16px; }
        .dar-value { overflow: hidden; color: #f0d8c8; font-size: 12px; line-height: 18px; text-overflow: ellipsis; white-space: nowrap; text-shadow: 2px 2px 0 rgba(0, 0, 0, 0.6); }
        .dar-description { min-height: 16px; margin: -2px 0 10px; color: #b98a76; font-size: 10px; line-height: 16px; }
        .dar-track { position: relative; display: grid; align-items: end; height: 32px; border: 2px solid #3a1c14; border-radius: 0; background: #120908; box-shadow: inset 0 0 0 2px #120908; image-rendering: pixelated; }
        .dar-cells { position: absolute; inset: 5px; display: flex; gap: 2px; pointer-events: none; }
        .dar-cell { flex: 1; min-width: 4px; border: 1px solid #3a352f; background: #282420; }
        ...
        .dar-range { position: relative; z-index: 1; width: 100%; height: 32px; margin: 0; cursor: pointer; }
        .dar-range--hidden { opacity: 0; }
        .dar-range:disabled { cursor: progress; }
        .dar-labels { display: flex; justify-content: space-between; margin: 0 2px 5px; color: #b98a76; font-size: 10px; line-height: 14px; pointer-events: none; }
        ...
        .dar-status { padding: 8px 0 0; color: #b98a76; font-size: 11px; line-height: 16px; }
        .dar-error { color: #ff6a4a; }
        .dar-declare { margin-top: 10px; padding: 8px; border: 2px solid #3a1c14; background: #1a0d0a; }
        .dar-declareHint { display: block; margin-bottom: 8px; color: #b98a76; font-size: 10px; line-height: 16px; }
        .dar-declareRow { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
        .dar-declareButton { flex: 0 0 auto; padding: 5px 10px; border: 2px solid #6a2c1e; border-radius: 0; background: #2a1410; color: #f0d8c8; font: inherit; font-size: 11px; cursor: pointer; }
        .dar-declareButton:hover:not(:disabled) { background: #3a1c14; }
        .dar-declareButton:disabled { color: #6a4a3e; cursor: progress; }
        .dar-menu-entry { display: flex; align-items: center; gap: 8px; width: 100%; min-height: 34px; margin-top: 4px; padding: 6px 8px; border: none; border-radius: 0; background: #1a0d0a; color: #f0d8c8; font-family: 'PressStart2P', 'Zpix', 'Courier New', monospace; font-size: 11px; line-height: 16px; text-align: left; cursor: pointer; }
...
void main(){
  ...
  vec3 ember = vec3(0.55, 0.06, 0.03);
  vec3 red   = vec3(1.0, 0.15, 0.08);
  vec3 tone  = mix(ash, ember, smoothstep(0.1, 0.45, heat));
  tone       = mix(tone, red, smoothstep(0.4, 0.9, heat));
  float temp = 1.0 - dn;
  vec3 col = tone * (0.35 + 0.65 * temp);
  col *= total;
  float pulse = sin(t * 2.8) * 0.15 + 1.0;
  float core  = exp(-pow((uv.x - u_slider) * 16.0, 2.0));
  col += red * core * 2.2 * pulse * intensity * es;
  col += ember * exp(-pow((uv.x - u_slider) * 3.5, 2.0)) * 0.12 * intensity * es;
  col *= cell;
  col *= fade_mask;
...
`
    ...
    function createWebglFire(canvas, getSlider, getActive) {
      ...
      function ensureLoop() {
        ...
        loopRunning = true
        idleFrames = 0
        // Start the clock 3s in the past so the shader's ignition animation
        // is already complete: the fire burns at full length immediately
        // instead of appearing to ignite only after interaction.
    ...
    function EffortPanel({ api, remote, sessionId, close }) {
      ...
      // Per-level ember palette: ash grey at the lowest level, burning red
      // at the highest. Lit cells glow brighter than unlit ones.
      const cellStyle = (cellIndex) => {
        const t = efforts.length > 1 ? cellIndex / (efforts.length - 1) : 0
        const on = cellIndex <= displayIndex
        const s = 8 + t * 90
        const l = on ? 34 + t * 26 : 18 + t * 20
        return {
          background: `hsl(6, ${s.toFixed(0)}%, ${l.toFixed(0)}%)`,
          borderColor: `hsl(6, ${s.toFixed(0)}%, ${(l + (on ? 14 : 6)).toFixed(0)}%)`,
          boxShadow: on ? `inset 0 0 3px hsla(12, 100%, ${(55 + t * 25).toFixed(0)}%, ${(0.25 + t * 0.45).toFixed(2)})` : 'none',
        }
      }
      const valueT = efforts.length > 1 ? displayIndex / (efforts.length - 1) : 0
      const valueColor = `hsl(6, ${(8 + valueT * 90).toFixed(0)}%, ${(55 + valueT * 25).toFixed(0)}%)`
      return React.createElement('div', { className: 'dar-panel', role: 'dialog', 'aria-label': copy.title },
        React.createElement('div', { className: 'dar-heading' },
          React.createElement('span', { className: 'dar-title' }, copy.title),
