  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector(`style[data-plugin-css="${STYLE_ID}"]`) === null) {
      ...
      style.textContent = `
        ...
        .dar-cell { flex: 1; min-width: 4px; background: #1f0e0a; border: 1px solid #301812; }
        .dar-cell-on { background: #8a2a12; border-color: #c84a26; box-shadow: inset 0 0 3px rgba(255, 122, 66, 0.55); }
...
void main(){
  ...
  vec3 ember = vec3(0.16, 0.006, 0.003);
  vec3 wmid  = vec3(0.60, 0.02, 0.015);
  vec3 wht   = vec3(1.0, 0.18, 0.12);
  float temp = 1.0 - dn;
  vec3 col   = mix(ember, wmid, temp);
  ...
  col += wmid * exp(-pow((uv.x - u_slider) * 3.5, 2.0)) * 0.12 * intensity * es;
  col *= cell;
  col *= fade_mask;
...
`
    ...
    function EffortPanel({ api, remote, sessionId, close }) {
      ...
      return React.createElement('div', { className: 'dar-panel', role: 'dialog', 'aria-label': copy.title },
        React.createElement('div', { className: 'dar-heading' },
          React.createElement('span', { className: 'dar-title' }, copy.title),
          React.createElement('span', { className: 'dar-value', 'aria-live': 'polite' }, level.name || label(level.id))),
        ...
        React.createElement('div', { className: `dar-track${dragging ? ' dar-dragging' : ''}` },
          React.createElement('div', { className: 'dar-cells', 'aria-hidden': true }, efforts.map((effort, cellIndex) => React.createElement('span', { className: `dar-cell${cellIndex <= displayIndex ? ' dar-cell-on' : ''}`, key: effort.id }))),
