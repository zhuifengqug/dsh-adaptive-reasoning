  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector(`style[data-plugin-css="${STYLE_ID}"]`) === null) {
      ...
      style.textContent = `
        ...
        .dar-track { position: relative; display: grid; align-items: end; height: 32px; overflow: hidden; border: 1px solid var(--dsw-alias-border-l2); border-radius: 8px; background: rgba(10, 8, 22, 0.5); }
        .dar-track::before { position: absolute; right: 0; bottom: 4px; left: 0; height: 3px; border-radius: 999px; background: var(--dsw-alias-border-l2); content: ''; }
        .dar-fill { position: absolute; bottom: 4px; left: 0; height: 3px; border-radius: 999px; background: var(--dsw-alias-state-info-primary); pointer-events: none; opacity: 0.6; transition: width 140ms ease; }
        .dar-track.dar-dragging .dar-fill { transition: none; }
        .dar-flame-canvas { position: absolute; inset: 0; width: 100%; height: 100%; mix-blend-mode: screen; pointer-events: none; }
        ...
        .dar-labels { display: flex; justify-content: space-between; margin: 0 2px 4px; color: var(--dsw-alias-label-caption); font-size: 11px; line-height: 16px; pointer-events: none; }
...
void main(){
  ...
  vec3 ember = vec3(0.30, 0.06, 0.02);
  vec3 wmid  = vec3(1.0, 0.45, 0.05);
  vec3 wht   = vec3(1.0, 0.97, 0.90);
  float temp = 1.0 - dn;
    ...
    function EffortPanel({ api, remote, sessionId, close }) {
      ...
      return React.createElement('div', { className: 'dar-panel', role: 'dialog', 'aria-label': copy.title },
        ...
        React.createElement('div', { className: `dar-track${dragging ? ' dar-dragging' : ''}` },
          React.createElement('div', { className: 'dar-fill', style: { width: `${(15 + 0.85 * rawValue).toFixed(2)}%` } }),
