  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector(`style[data-plugin-css="${STYLE_ID}"]`) === null) {
      ...
      style.textContent = `
        ...
        .dar-panel { width: min(262px, calc(100vw - 20px)); pointer-events: auto; padding: 11px 11px 10px; border: 2px solid #6a2c1e; border-radius: 0; background: #150a08; box-shadow: inset 0 0 0 2px #150a08, inset 0 0 0 4px #6a2c1e, 5px 5px 0 0 rgba(0, 0, 0, 0.55); color: #f0d8c8; font-family: 'PressStart2P', 'Zpix', 'Courier New', monospace; image-rendering: pixelated; }
        .dar-heading { display: flex; align-items: baseline; justify-content: space-between; gap: 9px; margin: 0 0 8px; }
        .dar-title { color: #b98a76; font-size: 9px; line-height: 13px; }
        .dar-value { overflow: hidden; color: #f0d8c8; font-size: 10px; line-height: 15px; text-overflow: ellipsis; white-space: nowrap; text-shadow: 2px 2px 0 rgba(0, 0, 0, 0.6); }
        .dar-description { min-height: 13px; margin: -2px 0 8px; color: #b98a76; font-size: 8px; line-height: 13px; }
        .dar-track { position: relative; display: grid; align-items: end; height: 27px; border: 2px solid #2e2926; border-radius: 0; background: #070606; box-shadow: inset 0 0 0 2px #070606; image-rendering: pixelated; }
        ...
        .dar-range { position: relative; z-index: 1; width: 100%; height: 27px; margin: 0; cursor: pointer; }
        .dar-range--hidden { opacity: 0; }
        .dar-range:disabled { cursor: progress; }
        .dar-labels { display: flex; justify-content: space-between; margin: 0 2px 4px; color: #b98a76; font-size: 8px; line-height: 11px; pointer-events: none; }
        .dar-tick { overflow: hidden; max-width: 48px; text-overflow: ellipsis; white-space: nowrap; }
        .dar-tick:first-child { text-align: left; }
        .dar-tick:last-child { text-align: right; }
        .dar-status { padding: 6px 0 0; color: #b98a76; font-size: 9px; line-height: 13px; }
        .dar-error { color: #ff6a4a; }
        .dar-declare { margin-top: 8px; padding: 6px; border: 2px solid #3a1c14; background: #1a0d0a; }
        .dar-declareHint { display: block; margin-bottom: 6px; color: #b98a76; font-size: 8px; line-height: 13px; }
        .dar-declareRow { display: flex; align-items: center; justify-content: space-between; gap: 6px; }
        .dar-declareButton { flex: 0 0 auto; padding: 4px 9px; border: 2px solid #6a2c1e; border-radius: 0; background: #2a1410; color: #f0d8c8; font: inherit; font-size: 9px; cursor: pointer; }
        .dar-declareButton:hover:not(:disabled) { background: #3a1c14; }
        .dar-declareButton:disabled { color: #6a4a3e; cursor: progress; }
        .dar-menu-entry { display: flex; align-items: center; gap: 7px; width: 100%; min-height: 30px; margin-top: 4px; padding: 5px 7px; border: none; border-radius: 0; background: #1a0d0a; color: #f0d8c8; font-family: 'PressStart2P', 'Zpix', 'Courier New', monospace; font-size: 9px; line-height: 13px; text-align: left; cursor: pointer; }
...
`
    ...
    function apply(ctx) {
      ...
      const show = (sessionId, anchor, menu = null) => {
        ...
        const width = Math.min(262, window.innerWidth - 20)
        const left = menu !== null ? rect.left : Math.max(10, Math.min(rect.right - width, window.innerWidth - width - 10))
        const top = menu !== null ? rect.top : (rect.top > 150 ? Math.max(10, rect.top - 146) : Math.min(window.innerHeight - 146, rect.bottom + 7))
        if (menu !== null) menu.style.display = 'none'
        host.style.left = `${Math.max(10, Math.min(left, window.innerWidth - width - 10))}px`
        host.style.top = `${Math.max(10, Math.min(top, window.innerHeight - 146))}px`
