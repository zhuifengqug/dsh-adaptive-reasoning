  ...
  factory: (require) => {
    ...
    if (typeof document !== 'undefined' && document.querySelector(`style[data-plugin-css="${STYLE_ID}"]`) === null) {
      ...
      style.textContent = `
        ...
        .dar-flame-canvas { position: absolute; left: 9px; right: 9px; top: 0; bottom: 0; width: calc(100% - 18px); height: 100%; pointer-events: none; }
        .dar-flame-glow { position: absolute; right: 9px; bottom: 4px; left: 9px; height: 14px; border-radius: 999px; opacity: 0; pointer-events: none; filter: blur(8px); transition: opacity 160ms ease; }
    ...
    class FlameRenderer {
      ...
      draw(staticOnly) {
        ...
        // Fuel slot.
        const slotColor = readCssColor('--dsw-alias-border-l2') || 'rgba(128,128,128,0.35)'
        this.roundRect(trackLeft, baseY - 2, trackWidth, 4, 2, slotColor)
    ...
    function EffortPanel({ api, remote, sessionId, close }) {
      ...
      // Initialise flame renderer once the canvas exists.
      React.useEffect(() => {
        if (canvasRef.current === null) return
        const renderer = new FlameRenderer(canvasRef.current)
      ...
      return React.createElement('div', { className: 'dar-panel', role: 'dialog', 'aria-label': copy.title },
        ...
        React.createElement('div', { className: 'dar-track' },
          React.createElement('canvas', { className: 'dar-flame-canvas', ref: canvasRef, 'aria-hidden': true }),
